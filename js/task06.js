...
// Если хочешь, в случае некорректного ввода, показывай alert с текстом 'Было введено не число, попробуйте еще раз', при этом результат prompt записывать в массив чисел не нужно,
...
const sumArray = function () {
  let total = 0;
  for (let i = 0; i < strToNum.length; i += 1) {
    total += strToNum[i];
  }
...
numbers.pop();
let strToNum = numbers.map(Number);
sumArray(strToNum);
...
// alert(
//   `Общая сумма чисел равна ${numbers.reduce((cur, next) => cur + next, 0)}`
// );
